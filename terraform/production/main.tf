...
terraform {
  backend "s3" {
    bucket                  = ""
    key                     = "project-one-production/terraform.tfstate"
    region                  = "ap-northeast-1"
    shared_credentials_file = "./.aws"
  }
}
...
#Use this config if the Nextjs is the new one in AWS account
#Disable by default, please uncomment when get the request for Production
/*
module "waf" {
  source                       = "../modules/waf"
  name                         = var.name
...
module "notify_slack" {
  ...
  slack_webhook_url = "https://hooks.slack.com/services/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"
  slack_channel     = "project_one_github"
  slack_username    = "AWS"
}
...
module "cloudwatch-logs-exporter" {
  ...
  s3_bucket = local.cloudwatch_log_groups
...
module "codebuild-log" {
  ...
  s3_bucket = local.codebuild_bucket_name
  schedule  = "cron(0 10 * * ? *)"
}
